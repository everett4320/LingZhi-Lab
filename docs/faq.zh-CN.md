...
Lingzhi Lab 常见的安装和运行问题，采用**问题 → 原因 → 解决方案**格式。另请参阅[README](../README.zh-CN.md)和[配置参考](./configuration.zh-CN.md)。
...
**解决方案：** 先检查该环境变量是否被设置，然后在启动 Lingzhi Lab 的那一层覆盖或移除它。

```sh
echo "${CODEX_SANDBOX_NETWORK_DISABLED:-0}"
```

如果命令输出 `1`，请在 shell 配置、systemd、Docker、PM2 或其他启动配置中移除或覆盖该变量，然后重启 Lingzhi Lab。
