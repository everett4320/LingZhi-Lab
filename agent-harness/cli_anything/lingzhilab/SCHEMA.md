...
This document defines the stable JSON payloads produced by `lingzhilab` for OpenClaw, mobile, and voice clients.
...
Used in:
- `lingzhilab --json chat send`
- `lingzhilab --json chat reply`
- `lingzhilab --json workflow continue`
- `lingzhilab --json workflow resume`
  ...
  "next_actions": [
    {
      ...
      "label": "Reply",
      "kind": "command",
      "command": "lingzhilab --json chat reply ..."
    }
  ],
  "waiting_sessions": []
}
...
Used in:
- `lingzhilab --json workflow status`
- `lingzhilab --json digest project`
...
Used in:
- `lingzhilab --json openclaw report --project ... --dry-run`
...
`lingzhilab openclaw-watch on` starts a background daemon that:
...
- stores watcher state in `~/.lingzhilab/openclaw-watcher-state.json`
