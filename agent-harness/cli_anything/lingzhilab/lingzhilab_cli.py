"""
lingzhilab - CLI harness for the Lingzhi Lab / LingzhiLab research workspace.

Entry point: cli_anything.lingzhilab.lingzhilab_cli:cli

Usage overview:
  lingzhilab [--json] [--url URL] <command> [<subcommand>] [options]

Global flags (must come before the command):
  --json        Output all results as JSON to stdout.
  --url URL     Override the Lingzhi Lab server URL for this invocation.

Sub-command tree:
  auth
    login       Authenticate and store token in ~/.lingzhilab_session.json
  ...
  openclaw
    install     Install the Lingzhi Lab skill into OpenClaw
...
_OPENCLAW_SKILL_DIR_NAME = "lingzhilab"
_SKILL_MD_FILENAME = "SKILL.md"
_HARNESS_ROOT = Path(__file__).parent.parent.parent
_OPENCLAW_SKILL_SOURCE_DIR = _HARNESS_ROOT / "skills" / "lingzhi-lab"
_PRIMARY_CLI_NAME = "lingzhilab"
...
class Context:
    def __init__(self, json_mode: bool, client: LingzhiLab, lang: str = "en") -> None:
...
def _handle_error(exc: Exception, json_mode: bool) -> None:
    ...
    if isinstance(exc, NotLoggedInError):
        ...
        error(f"Could not connect to the Lingzhi Lab server. Is it running?  ({exc})")
...
def _resolve_project_ref(
    ...
    if not isinstance(projects, list):
        raise ValueError("Failed to load LingzhiLab projects.")
    ...
    raise ValueError("No LingzhiLab projects are available.")
...
def _require_project_name(project: Dict[str, Any], project_ref: str) -> str:
    ...
    if not project_name:
        raise ValueError(
            f"Project '{project_ref}' is not registered in Lingzhi Lab. Add it first, or choose a project from `{_cli_cmd('projects list')}`."
        )
...
def _resolve_current_lingzhilab_bin() -> str:
    candidates = [
        shutil.which(_PRIMARY_CLI_NAME),
        shutil.which("lingzhi-lab"),
    ...
    for name in (_PRIMARY_CLI_NAME, "lingzhi-lab", "vibelab"):
        if user_base:
    ...
    *,
    ...
    if not skill_source.exists():
        raise FileNotFoundError(
            f"Lingzhi Lab skill source not found at {skill_source}. Run this command from the agent-harness repo."
        )
    ...
    session_data["openclaw_lingzhilab_bin"] = _resolve_current_lingzhilab_bin()
    ...
    return {
        ...
        "lingzhilab_bin": session_data.get("openclaw_lingzhilab_bin") or _resolve_current_lingzhilab_bin(),
    }
...
def _build_openclaw_report(
    ...
    header = click.style(f"📊 [Lingzhi Lab] {project_label}", bold=True, fg="cyan")
...
def _build_openclaw_chat_notification(payload: Dict[str, Any], action: str, ctx: Context) -> str:
    ...
    title = click.style(f"💬 [Lingzhi Lab] {project_name}", bold=True, fg="cyan")
    lines = [title]
...
def _format_portfolio_digest(payload: Dict[str, Any]) -> str:
    ...
    header = click.style("📋 [Lingzhi Lab Portfolio Digest]", bold=True, fg="cyan")
    lines = [header]
...
def _format_project_progress(payload: Dict[str, Any]) -> str:
    ...
    title = click.style(f"🚀 [Lingzhi Lab Project Progress] {payload.get('project_display_name') or payload.get('project')}", bold=True, fg="cyan")
    lines = [title]
...
def _format_project_digest(payload: Dict[str, Any]) -> str:
    ...
    title = click.style(f"📑 [Lingzhi Lab Digest] {payload.get('project_display_name') or payload.get('project')}", bold=True, fg="cyan")
    lines = [title]
...
def _format_daily_digest(payload: Dict[str, Any]) -> str:
    ...
    header = click.style("📅 [Lingzhi Lab Daily Digest]", bold=True, fg="cyan")
    lines = [header]
...
def _maybe_send_openclaw_chat_notification(
    ...
    if not resolved_channel:
        raise ValueError(
            "OpenClaw notification requested, but no channel is configured. Use --notify-to <channel> or run `lingzhilab openclaw configure --push-channel <channel>` first."
        )
...
@click.option("--url", "url_override", default=None, metavar="URL", help="Override the Lingzhi Lab server URL.")
@click.option("--lang", "lang", default=None, help="Language for output (en, zh). Defaults to en or LINGZHILAB_LANG.")
@click.pass_context
def cli(ctx: click.Context, json_mode: bool, url_override: Optional[str], lang: Optional[str]) -> None:
    """Lingzhi Lab CLI harness - manage projects, sessions, workflows, and OpenClaw integration."""
    effective_lang = lang or os.environ.get("LINGZHILAB_LANG") or "en"
    client = LingzhiLab(url_override=url_override)
...
def vibelab_cli():
    """Alias for lingzhilab with a deprecation warning."""
    warnings.warn(
        "The 'vibelab' command is deprecated and will be removed in a future version. "
        "Please use 'lingzhilab' instead.",
        DeprecationWarning,
        stacklevel=1
    )
    # Print warning to stderr as well since DeprecationWarning is often filtered
    print("Warning: 'vibelab' command is deprecated; use 'lingzhilab' instead.", file=sys.stderr)
    cli()
...
@click.option("--lingzhilab-bin", default=None, metavar="PATH", help="CLI binary used by the watcher subprocess.")
@pass_context
def openclaw_watch_on(ctx: Context, channel: Optional[str], interval: int, lingzhilab_bin: Optional[str]) -> None:
    """Start the OpenClaw watcher daemon."""
    try:
        resolved_channel = _resolve_push_channel(channel)
        if not resolved_channel:
            raise ValueError("No channel specified. Use --to <channel> or run `lingzhilab openclaw configure --push-channel <channel>` first.")
        payload = openclaw_daemon_mod.watcher_start(
            channel=resolved_channel,
            interval=interval,
            lingzhilab_bin=lingzhilab_bin or _resolve_current_lingzhilab_bin(),
...
@auth.command("login")
@click.option("--username", "-u", prompt="Username", help="LingzhiLab username.")
@click.option("--password", "-p", prompt="Password", hide_input=True, help="LingzhiLab password.")
@pass_context
...
def projects_add(ctx: Context, project_path: str, display_name: Optional[str]) -> None:
    """Register PROJECT_PATH as a LingzhiLab project."""
    try:
...
def projects_create(ctx: Context, workspace_path: str, display_name: Optional[str], github_url: Optional[str]) -> None:
    """Create WORKSPACE_PATH as a new LingzhiLab project workspace."""
    try:
...
@cli.group()
def server() -> None:
    """Start / stop the Lingzhi Lab Node.js server as a background daemon."""


@server.command("on")
@click.option("--path", "server_path", default=None, metavar="PATH", help="Path to the LingzhiLab installation directory (saved for future use).")
...
def server_on(ctx: Context, server_path: Optional[str], port: Optional[int]) -> None:
    """Start the Lingzhi Lab server as a daemon. Logs -> ~/.vibelab/logs/server.log"""
    try:
...
def server_off(ctx: Context) -> None:
    """Stop the running LingzhiLab daemon."""
    try:
...
def chat_waiting(ctx: Context, project_ref: Optional[str], provider: Optional[str], force_json: bool) -> None:
    ...
    try:
        ...
        output(compact, json_mode=ctx.json_mode, title="Lingzhi Lab Waiting Sessions")
    except Exception as exc:
        _handle_error(exc, ctx.json_mode)
...
def install_command(ctx: Context, openclaw_dir: Optional[str], server_url: Optional[str], push_channel: Optional[str]) -> None:
    """One-command setup for Lingzhi Lab + OpenClaw local integration."""
    try:
        ...
        payload["next_steps"] = [
            f"{payload['lingzhilab_bin']} --url {payload['server_url']} --json chat waiting",
            f"{payload['lingzhilab_bin']} --url {payload['server_url']} --json digest portfolio",
            "~/.openclaw/workspace/skills/lingzhilab/scripts/openclaw_lingzhilab_turn.sh --json -m 'Use your exec tool to run `$LINGZHILAB_BIN --url \"$VIBELAB_URL\" chat waiting --json`. Return a concise Chinese summary.'",
        ]
        if ctx.json_mode:
            output(payload, json_mode=True)
        else:
            success(f"Lingzhi Lab linked to OpenClaw at {payload['skill_dir']}", json_mode=False)
            info(f"Server URL : {payload['server_url']}")
            info(f"CLI path   : {payload['lingzhilab_bin']}")
...
def openclaw_install(ctx: Context, openclaw_dir: Optional[str], server_url: Optional[str], push_channel: Optional[str]) -> None:
    """Install the Lingzhi Lab skill into the OpenClaw workspace."""
    try:
        payload = _install_openclaw_skill(
            openclaw_dir=openclaw_dir,
            server_url=server_url or ctx.client.get_base_url(),
        ...
        if ctx.json_mode:
            ...
            success(f"Lingzhi Lab skill installed to {payload['skill_dir']}", json_mode=False)
            info(f"Server URL : {payload['server_url']}")
            info(f"CLI path   : {payload['lingzhilab_bin']}")
            if payload.get("push_channel"):
                info(f"Push chan  : {payload['push_channel']}")
...
@click.option("--to", "channel", default=None, metavar="CHANNEL", help="Destination channel (for example feishu:<chat_id>). Falls back to saved openclaw_push_channel in ~/.lingzhilab_session.json.")
@pass_context
def openclaw_push(ctx: Context, message_text: str, channel: Optional[str]) -> None:
    """Send a message via the OpenClaw CLI."""
    resolved_channel = _resolve_push_channel(channel)
    if not resolved_channel:
        error("No channel specified. Use --to <channel> or run `lingzhilab openclaw configure --push-channel <channel>` first.")
        sys.exit(1)
...
def openclaw_configure(ctx: Context, push_channel: str) -> None:
    """Save OpenClaw integration settings to ~/.lingzhilab_session.json."""
    session_file = SESSION_FILE
    session_data = _load_session_file(session_file)
    session_data["openclaw_push_channel"] = push_channel
...
def openclaw_report(
    ...
    try:
        ...
        if not dry_run:
            if not resolved_channel:
                raise ValueError(
                    "No channel specified. Use --to <channel>, run `lingzhilab openclaw configure --push-channel <channel>`, or pass --dry-run."
                )
