"""Background OpenClaw watcher daemon driven by Lingzhi Lab WebSocket events."""
...
from .session import LingzhiLab, _load_session_file
...
LINGZHILAB_DIR = Path.home() / ".lingzhilab"
LEGACY_VIBELAB_DIR = Path.home() / ".vibelab"

BASE_DIR = LINGZHILAB_DIR
if not LINGZHILAB_DIR.exists() and LEGACY_VIBELAB_DIR.exists():
...
def watcher_start(
    *,
    ...
    interval: int = 30,
    lingzhilab_bin: Optional[str] = None,
    ...
    args = [
        sys.executable,
        "-m",
        "cli_anything.lingzhilab.core.openclaw_daemon",
        "run",
        "--interval",
        str(max(interval, 5)),
    ]
    ...
    if lingzhilab_bin:
        args.extend(["--lingzhilab-bin", lingzhilab_bin])
    if base_url:
...
def _build_project_digest(client: LingzhiLab, project: Dict[str, Any]) -> Dict[str, Any]:
    project_name = str(project.get("name") or "").strip()
    if not project_name:
        raise ValueError("Project is not registered in Lingzhi Lab.")
...
def _resolve_project_from_event(client: LingzhiLab, event: Dict[str, Any], cached_projects: Optional[List[Dict[str, Any]]] = None) -> Optional[Dict[str, Any]]:
...
def _build_summary_prompt(event: Dict[str, Any], project_digest: Dict[str, Any], signals: List[Dict[str, Any]]) -> str:
    ...
    return (
        "请作为 Lingzhi Lab 项目秘书，用中文给用户发送一条飞书通知。\n"
        "要求：\n"
...
def _build_event_message(event: Dict[str, Any], portfolio_event: Optional[Dict[str, Any]]) -> str:
    ...
    lines = [f"[Lingzhi Lab] {project_name}", f"Type: {mapped_kind}"]
...
def _watch_loop(channel: str, lingzhilab_bin: str, base_url: Optional[str], interval: int) -> None:
...
def run_loop(*, channel: str, interval: int, lingzhilab_bin: str, base_url: Optional[str]) -> None:
    _append_log(f"[watcher] Run loop starting at {_now()} channel={channel} interval={interval}")
    _watch_loop(channel=channel, lingzhilab_bin=lingzhilab_bin, base_url=base_url, interval=interval)


def main(argv: Optional[List[str]] = None) -> int:
    import argparse

    parser = argparse.ArgumentParser(description="Lingzhi Lab OpenClaw watcher daemon")
    subparsers = parser.add_subparsers(dest="command", required=True)

    run_parser = subparsers.add_parser("run")
    run_parser.add_argument("--channel", required=False)
    run_parser.add_argument("--interval", type=int, default=30)
    run_parser.add_argument("--lingzhilab-bin", default="lingzhilab")
    ...
    if args.command == "run":
        ...
        _ensure_dirs()
        run_loop(channel=channel, interval=args.interval, lingzhilab_bin=args.lingzhilab_bin, base_url=args.base_url)
    return 0
