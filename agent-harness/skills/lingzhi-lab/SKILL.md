---
name: lingzhi-lab
description: Lingzhi Lab skill for OpenClaw project discovery, idea intake, waiting-session triage, structured session control, event-driven notifications, and mobile reporting through the local lingzhilab CLI.
---

# Lingzhi Lab for OpenClaw

Use this skill when OpenClaw needs to operate Lingzhi Lab from chat or mobile, especially for:
- listing Lingzhi Lab projects
...
Before running Lingzhi Lab commands:

```bash
$LINGZHILAB_BIN server status
```

If the server is not running:

```bash
$LINGZHILAB_BIN server on
```

Assume the local wrapper exports these defaults when OpenClaw runs the skill:

```bash
LINGZHILAB_BIN="${LINGZHILAB_BIN:-$(which lingzhilab)}"
LINGZHILAB_URL=http://localhost:3001
```

When invoking the CLI from OpenClaw, prefer `$LINGZHILAB_BIN --url "$LINGZHILAB_URL" ...` instead of relying on PATH.
...
```bash
cat "$(git rev-parse --show-toplevel)/agent-harness/cli_anything/lingzhilab/SCHEMA.md"
```

When calling OpenClaw locally from automation or shell, use:

```bash
./scripts/openclaw_lingzhilab_turn.sh
```
...
```bash
$LINGZHILAB_BIN --url "$LINGZHILAB_URL" projects list
```

Inspect the latest message in a project:

```bash
$LINGZHILAB_BIN --url "$LINGZHILAB_URL" projects latest <project> --json
```

Inspect project progress and next actions:

```bash
$LINGZHILAB_BIN --url "$LINGZHILAB_URL" projects progress <project> --json
```

Create a new empty project workspace:

```bash
$LINGZHILAB_BIN --url "$LINGZHILAB_URL" projects create /absolute/path/to/project --name "Display Name" --json
```

Create a new project from a fresh idea and immediately start discussion:

```bash
$LINGZHILAB_BIN --url "$LINGZHILAB_URL" projects idea /absolute/path/to/project --name "Display Name" --idea "<idea text>" --json
```
...
```bash
$LINGZHILAB_BIN --url "$LINGZHILAB_URL" chat sessions --project <project> --json
```

List waiting sessions across all projects or one project:

```bash
$LINGZHILAB_BIN --url "$LINGZHILAB_URL" chat waiting --json
$LINGZHILAB_BIN --url "$LINGZHILAB_URL" chat waiting --project <project> --json
```
...
```bash
$LINGZHILAB_BIN --url "$LINGZHILAB_URL" chat reply --project <project> --session <session-id> \
...
```bash
./scripts/lingzhilab_wait_until_clear.sh --project <project> --session <session-id>
```
...
```bash
$LINGZHILAB_BIN --url "$LINGZHILAB_URL" workflow status --project <project> --json
$LINGZHILAB_BIN --url "$LINGZHILAB_URL" workflow continue --project <project> --session <session-id> --bypass-permissions -m "<instruction>" --json
$LINGZHILAB_BIN --url "$LINGZHILAB_URL" workflow approve --project <project> --session <session-id> --json
$LINGZHILAB_BIN --url "$LINGZHILAB_URL" workflow reject --project <project> --session <session-id> -m "<reason>" --json
$LINGZHILAB_BIN --url "$LINGZHILAB_URL" workflow retry --project <project> --session <session-id> --json
$LINGZHILAB_BIN --url "$LINGZHILAB_URL" workflow resume --project <project> --session <session-id> --bypass-permissions --json
```
...
```bash
$LINGZHILAB_BIN --url "$LINGZHILAB_URL" workflow status --project <project> --json
$LINGZHILAB_BIN --url "$LINGZHILAB_URL" digest project --project <project> --json
```
...
```bash
$LINGZHILAB_BIN --url "$LINGZHILAB_URL" digest daily --json
```
...
```bash
$LINGZHILAB_BIN --url "$LINGZHILAB_URL" digest portfolio --json
```
...
```bash
$LINGZHILAB_BIN --url "$LINGZHILAB_URL" workflow status --project <project> --json
$LINGZHILAB_BIN --url "$LINGZHILAB_URL" taskmaster artifacts --project <project> --json
```
...
```bash
$LINGZHILAB_BIN --url "$LINGZHILAB_URL" --json openclaw-watch on --to feishu:<chat_id>
$LINGZHILAB_BIN --url "$LINGZHILAB_URL" --json openclaw-watch status
$LINGZHILAB_BIN --url "$LINGZHILAB_URL" --json openclaw-watch off
```
...
1. Run `$LINGZHILAB_BIN --url "$LINGZHILAB_URL" projects list`.
2. Present short names, display names, and paths only when needed.

Pattern: user asks what needs attention
1. Run `$LINGZHILAB_BIN --url "$LINGZHILAB_URL" digest portfolio --json`.
...
1. Run `$LINGZHILAB_BIN --url "$LINGZHILAB_URL" chat reply --project ... --session ... -m ... --json`.
...
5. Optionally run `lingzhilab_wait_until_clear.sh` and report the final clearance.

Pattern: user suddenly has a new idea
1. Pick a workspace path, usually `/Users/<user>/vibelab/<slug>`.
2. Run `$LINGZHILAB_BIN --url "$LINGZHILAB_URL" projects idea <path> --name <display-name> --idea <idea> --json`.
3. Return the created project, session id, and first Lingzhi Lab reply.
4. Continue the discussion with `$LINGZHILAB_BIN --url "$LINGZHILAB_URL" chat reply` on that session.

Pattern: user wants an update without opening Lingzhi Lab
1. Run `$LINGZHILAB_BIN --url "$LINGZHILAB_URL" digest daily --json`, `$LINGZHILAB_BIN --url "$LINGZHILAB_URL" digest project --project ... --json`, or `$LINGZHILAB_BIN --url "$LINGZHILAB_URL" digest portfolio --json`.
